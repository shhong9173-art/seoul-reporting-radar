...
# Calculate a conservative pitch score: reward information gaps, penalize already-spread stories.
for x in items:
    text=(x.get('title','')+' '+x.get('summary','')).lower()
    signal=sum(w.lower() in text for w in strong)
    if x.get('global'):
        company=set(x.get('companies') or [])
        domestic=[d for d in items if not d.get('global') and company.intersection(d.get('companies') or []) and d.get('category')==x.get('category')]
        if not domestic: score=92+min(5,signal)
        elif len(domestic)==1: score=78+min(5,signal)
        else: score=48+min(8,signal)
        reasons=['해외 주요 매체 신호','국내 동일 분야 보도 공백' if not domestic else f'국내 관련 보도 {len(domestic)}건']
    else:
...
# Preserve strong discoveries for 90 days.
