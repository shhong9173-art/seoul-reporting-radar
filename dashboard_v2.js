(function(){
  ...
  async function render(){
    ...
    let pitches=[],changeData={changes:[]};
    try{const r=await fetch('pitch.json?v='+Date.now(),{cache:'no-store'});const j=await r.json();pitches=Array.isArray(j)?j:[]}catch(e){}
    try{const r=await fetch('changes.json?v='+Date.now(),{cache:'no-store'});const j=await r.json();changeData=j&&Array.isArray(j.changes)?j:{changes:[]}}catch(e){}
    ...
    const changes=(changeData.changes||[]).filter(x=>x.company).slice(0,6);
    ...
    box.innerHTML=`<div class="dash-head"><div><div class="eyebrow">REPORTING CONTROL ROOM</div><h2>오늘 취재 상황판</h2><p>기사 수집이 아니라 변화·단독 가능성·취재 방향을 먼저 봅니다.</p></div><button class="dash-refresh" type="button">다시 계산</button></div>
    <div class="dash-grid">
      <section class="dash-section dash-feature"><div class="dash-section-head"><b>오늘 발제</b><span>최대 3개</span></div>${pitches.map((p,i)=>card(`발제 ${i+1}`,p.headline,p.angle||'자료를 연결해 확인할 기사',`핵심 숫자: ${(p.numbers||[]).slice(0,4).join(' · ')||'없음'}`,'pitch-highlight')).join('')||'<div class="dash-empty">오늘 바로 쓸 발제가 없습니다.</div>'}</section>
      ...
      <section class="dash-section"><div class="dash-section-head"><b>최근 누적된 변화</b><span>7일+ 누적 신호</span></div>${changes.map(x=>card(`${x.category||'산업'} · ${(x.signals||[]).join('·')||'변화'}`,x.company,`${x.headline||'최근 사업 신호 변화'} · 최근 24시간 ${x.count24h||0}건`,(x.newNumbers||[]).slice(0,4).join(' · ')||'추가 숫자 확인')).join('')||'<div class="dash-empty">아직 누적 변화가 없습니다.</div>'}</section>
      <section class="dash-section"><div class="dash-section-head"><b>경쟁지가 먼저 쓴 것</b><span>같은 기사 말고 다음 질문</span></div>${competition.map(g=>{const l=g.slice().sort((a,b)=>time(b)-time(a))[0],s=[...new Set(g.map(x=>x.sourceName).filter(Boolean))];return card(`${s.length}개 매체`,titleOf(l),`${s.join(', ')}에서 보도된 이슈. 추가 숫자·계약·투자·생산 변화를 확인.`,'추가 취재 여부 판단')}).join('')||'<div class="dash-empty">확산 이슈가 없습니다.</div>'}</section>
