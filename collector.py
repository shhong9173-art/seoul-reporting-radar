...
SOURCE_TIERS={'연합뉴스':5,'연합뉴스TV':5,'한국경제':5,'매일경제':5,'서울경제':5,'이데일리':5,'머니투데이':5,'전자신문':5,'조선비즈':5,'파이낸셜뉴스':4,'아시아경제':4,'뉴스1':4,'뉴시스':4,'더팩트':4,'오토타임즈':5,'오토뷰':4,'카가이':4,'오토트리뷴':4,'모터그래프':4,'탑라이더':3}
...
def get(url,timeout=18):
 req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 AutoIndustryDesk/2.1','Accept':'application/rss+xml,application/xml,text/xml,*/*'})
 with urllib.request.urlopen(req,timeout=timeout) as r:return r.read()

def clean(s):
 s=re.sub(r'<script.*?</script>|<style.*?</style>|<[^>]+>',' ',s or '',flags=re.I|re.S);return re.sub(r'\s+',' ',html.unescape(s)).strip()

def norm_title(t):
 t=re.sub(r'\s*[-|｜].*$','',t);return re.sub(r'[^0-9A-Za-z가-힣 ]',' ',t).lower().split()

def tokens(t):
 return {w for w in re.findall(r'[가-힣A-Za-z0-9]{2,}',t.lower()) if w not in TOPIC_STOP and not re.fullmatch(r'\d+',w)}
...
def classify_cluster(x):
 text=x['title']+' '+x.get('summary','');return company_list(text),tokens(x['title']),[w for w in HIGH if w.lower() in text.lower()]

def same_issue(a,b):
 ac,at,ai=classify_cluster(a);bc,bt,bi=classify_cluster(b);company_overlap=bool(set(ac)&set(bc));sim=len(at&bt)/max(1,len(at|bt));impact_overlap=bool(set(ai)&set(bi));return (company_overlap and sim>=0.28) or (sim>=0.45 and impact_overlap)

def build_clusters(items):
 clusters=[]
 for x in sorted(items,key=lambda z:z['published']):
  hit=next((c for c in clusters[-80:] if same_issue(x,c['head'])),None)
  if hit:hit['items'].append(x)
  else:clusters.append({'head':x,'items':[x]})
 return clusters

def extract_number(text):
 m=re.search(r'(\d[\d,.]*\s*(?:조원|억원|만대|대|GWh|억달러|만톤|톤|%))',text,re.I);return m.group(1) if m else None
...
def enrich(items):
 for ci,c in enumerate(build_clusters(items),1):
  members=sorted(c['items'],key=lambda z:z['published']);earliest=members[0];sources=list(dict.fromkeys(m.get('sourceName','') for m in members if m.get('sourceName')));companies=sorted(set(sum((company_list(m['title']+' '+m.get('summary','')) for m in members),[])))
  for x in members:
   text=(x['title']+' '+x.get('summary','')).lower();excl=is_exclusive_title(x['title']);high_count=sum(w.lower() in text for w in HIGH);follow_count=sum(w.lower() in text for w in FOLLOW);tier=SOURCE_TIERS.get(x.get('sourceName',''),3);same_issue_count=len(members);coverage=min(18,max(0,(same_issue_count-1)*4));competition=12 if len(sources)>=3 else (8 if len(sources)==2 else 0);age_h=max(0,(datetime.now(KST)-datetime.fromisoformat(x['published'])).total_seconds()/3600);fresh=max(0,8-int(age_h//12));score=44+min(28,high_count*4)+min(15,len(companies)*3)+(16 if excl else 0)+tier+competition+fresh;score=max(38,min(99,score-coverage));follow=follow_count>=1 and (bool(companies) or any(w in x['title'] for w in ['관세','리콜','파업','수주','공장']));priority='must' if excl or score>=78 else ('follow' if follow else 'normal')
   if excl:why='제목 형식상 단독·속보 후보입니다. 최초 보도 여부와 취재원·회사 공식 확인을 먼저 점검하세요.'
