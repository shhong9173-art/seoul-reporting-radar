...
def fetch(url):
    try:
        req=Request(url,headers={'User-Agent':'Mozilla/5.0 Seoul-Reporting-Radar/1.0'})
...
def collect(org,url):
    ...
    for href,title in re.findall(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>([\s\S]*?)</a>',body,re.I):
        t=clean(title);u=urljoin(url,href)
        ...
        seen.add(u);isfile=bool(FILE.search(u));promo=bool(PROMO.search(t)) and not isfile
        nums=NUM.findall(t)
        score=35+ (8 if KEY.search(t) else 0)+(5 if nums else 0)+(5 if isfile else 0)
        out.append({'id':hashlib.sha1(u.encode()).hexdigest()[:12],'level':'B','category':cat(t),'org':org,'date':datetime.date.today().isoformat(),'score':min(score,69),'title':('[첨부파일] ' if isfile else '')+t,'summary':f'{org} 공개정보에서 자동 발견된 자료. 공식 발표 자체를 단독 근거로 판정하지 않습니다.','why':'공개자료 자체는 단독 근거가 아닙니다. 과거자료·첨부파일·예산·계약·통계와 교차검증해야 합니다.','keyNumber':nums[0] if nums else '','tags':['자동수집',org,cat(t)]+(['첨부파일'] if isfile else []),'questions':['원자료와 첨부파일에 본문에 없는 수치가 있는가?','전년·전월 또는 기존 계획과 무엇이 달라졌는가?','예산·계약·통계 자료와 숫자가 일치하는가?','25개 자치구 전체 추세와 비교했을 때 이상치인가?'],'source':u,'attachments':[u] if isfile else [],'standaloneEligible':False})
    return out[:40]

items=[]
for org,url in SOURCES+DISTRICTS:items.extend(collect(org,url))
# Never seed fake S/A items. Only verified comparison evidence may later promote them.
items=items[:400]
