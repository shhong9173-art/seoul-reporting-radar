...
KEY=re.compile(r'(통계|데이터|현황|실적|지표|분석|예산|결산|재정|계약|입찰|보조금|지원금|사업비|고시|공고|행정예고|입법예고|감사|위원회|회의|인허가|사업계획|공모|주택|건축|도시계획|교통|복지|안전)',re.I)
FILE=re.compile(r'\.(pdf|hwp|hwpx|xlsx?|csv|docx?|pptx?|zip)(?:[?#]|$)',re.I)
PROMO=re.compile(r'(보도자료|보도설명자료|행사|축제|캠페인|수상|업무협약|협약식|개최|참여|기념|홍보)',re.I)
NUM=re.compile(r'\d+(?:,\d{3})*(?:\.\d+)?\s*(?:조원|억원|만원|원|%|명|가구|세대|건|곳|개)',re.I)
DATE=re.compile(r'(20\d{2})[.\-/](\d{1,2})[.\-/](\d{1,2})')
LINK=re.compile(r'''<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)</a>''',re.I)
JUNK_TITLE=re.compile(r'(\.layer_popup|position\s*:|display\s*:|width\s*:|height\s*:|z-index|@media|font-size|margin-|padding-|box-sizing|background\s*:|text-indent|javascript:|mailto:|tel:|document\.|window\.|var\s+|function\s*\()',re.I)
NAV=re.compile(r'(주메뉴|사이트맵|로그인|회원가입|통합검색|바로가기|개인정보처리방침|이용약관|닫기|검색어입력|모바일 메뉴)',re.I)
PAGE_TIMEOUT=7;MAX_PAGE_BYTES=2_000_000;MAX_CANDIDATES_PER_ORG=8;MAX_DETAILS_PER_ORG=4;MAX_ITEMS=400

def fetch(url,timeout=PAGE_TIMEOUT):
    try:
        req=Request(url,headers={'User-Agent':'Mozilla/5.0 Seoul-Reporting-Radar/5.1','Accept':'text/html,application/xhtml+xml,*/*'})
...
def dedupe_text(s):
    chunks=[re.sub(r'\s+',' ',x).strip() for x in re.split(r'(?<=[.!?다요함됨])\s+',s) if x.strip()]
    seen=set();out=[]
    for c in chunks:
        if len(c)<25 or c in seen:continue
        seen.add(c);out.append(c)
    return ' '.join(out)

def bad_title(t):
    t=re.sub(r'\s+',' ',t).strip();low=t.lower()
    if len(t)<8 or len(t)>220:return True
    if JUNK_TITLE.search(t):return True
    if t.startswith(('.', '#', '@')):return True
    if t.count('{')+t.count('}')>=2 or t.count(';')>=2:return True
    if len(re.findall(r'[가-힣]',t))<3:return True
    return False

def bad_link(u):
    return urlparse(u).scheme not in ('http','https')

def content_text(body):
    body=re.sub(r'<script[\s\S]*?</script>|<style[\s\S]*?</style>|<noscript[\s\S]*?</noscript>',' ',body,flags=re.I)
    candidates=[]
    patterns=[
      r'<article\b[^>]*>[\s\S]*?</article>',
      r'<div[^>]+(?:id|class)=["\'][^"\']*(?:board[_-]?view|bbs[_-]?view|view[_-]?(?:cont|content|body)|article[_-]?(?:body|content)|board[_-]?content|detail[_-]?(?:content|view))[^"\']*["\'][^>]*>[\s\S]*?</div>',
      r'<section[^>]+(?:id|class)=["\'][^"\']*(?:view|article|board|bbs)[^"\']*["\'][^>]*>[\s\S]*?</section>'
    ]
    for pat in patterns:
        for m in re.finditer(pat,body,re.I):
            t=dedupe_text(clean_html(m.group(0)))
            if 120<=len(t)<=20000:
                penalty=40*len(NAV.findall(t))+100*len(re.findall(r'(?:position|z-index|@media|font-size|padding|margin)\s*:',t,re.I))
                score=2*len(re.findall(r'[가-힣]',t))+3*len(NUM.findall(t))-penalty
                candidates.append((score,t))
    if candidates:
        return max(candidates,key=lambda z:z[0])[1][:12000]
    t=dedupe_text(clean_html(body))
    if len(NAV.findall(t))>=8 and len(t)>1500:
        # Remove obvious global navigation fragments from a whole-page fallback.
        words=re.split(r'\s+',t)
        kept=[w for w in words if not NAV.search(w)]
        t=' '.join(kept)
    return t[:12000]
...
def links(page,base):
    ...
    for href,title in LINK.findall(page):
        ...
        t=clean_html(title)
        if bad_link(u) or bad_title(t):continue
...
def detail(url):
    ...
    return text,list(dict.fromkeys(atts))[:6],pubdate
...
def collect(org,url):
    ...
    for href,title in links(page,url):
        ...
        if NAV.search(title) and len(re.findall(r'[가-힣]',title))<15:continue
        if KEY.search(title) or FILE.search(href) or PROMO.search(title):
            seen.add(href);candidates.append((href,title))
    candidates.sort(key=lambda x:(10 if KEY.search(x[1]) else 0)+(6 if NUM.search(x[1]) else 0)+(4 if FILE.search(x[0]) else 0),reverse=True)
    ...
    for href,title in candidates:
        bodytext,atts,pubdate=details.get(href,('',[],''));allatts=[href] if FILE.search(urlparse(href).path) else [x for x in atts if FILE.search(urlparse(x).path)]
