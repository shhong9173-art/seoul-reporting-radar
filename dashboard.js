(function(){
  ...
  async function render(){
    ...
    box.innerHTML=`<div class="dash-head"><div><div class="eyebrow">REPORTING CONTROL ROOM</div><h2>오늘 취재 상황판</h2><p>오늘 무엇을 쓰고, 무엇이 달라졌는지를 먼저 봅니다.</p></div><button class="dash-refresh" type="button">다시 계산</button></div>
    <div class="dash-grid">
      <section class="dash-section dash-feature"><div class="dash-section-head"><b>오늘 발제</b><span>최대 3개</span></div>${pitches.map((p,i)=>card(`발제 ${i+1}`,p.headline,p.angle||'취재 방향을 확인하세요.',(p.numbers||[]).slice(0,4).join(' · '),'pitch-highlight')).join('')||'<div class="dash-empty">오늘 바로 쓸 발제가 없습니다.</div>'}</section>
      ...
      <section class="dash-section"><div class="dash-section-head"><b>최근 누적된 변화</b><span>최근 31일 누적 신호</span></div>${changes.map(x=>card(`${x.category||'산업'} · ${(x.signals||[]).join('·')||'변화'}`,x.company,`${x.headline||'최근 사업 신호가 달라졌습니다.'} · 최근 24시간 ${x.count24h||0}건`,(x.newNumbers||[]).slice(0,4).join(' · ')||'추가 숫자 확인')).join('')||'<div class="dash-empty">아직 누적 변화가 없습니다. 수집이 쌓이면 정확도가 높아집니다.</div>'}</section>
      <section class="dash-section"><div class="dash-section-head"><b>경쟁지가 먼저 쓴 것</b><span>같은 기사 말고 다음 사실</span></div>${competition.map(g=>{const l=g.slice().sort((a,b)=>time(b)-time(a))[0],s=[...new Set(g.map(x=>x.sourceName).filter(Boolean))];return card(`${s.length}개 매체`,titleOf(l),`${s.join(', ')}에서 보도된 이슈. 추가 숫자·계약·투자·생산 변화를 확인.`,'추가 취재 여부 판단')}).join('')||'<div class="dash-empty">확산 이슈가 없습니다.</div>'}</section>
      ...
      <section class="dash-section"><div class="dash-section-head"><b>산업별 변화</b><span>기사 수보다 변화폭</span></div><div class="dash-industries">${ind.map(x=>`<button type="button" data-cat="${esc(x.c)}"><strong>${esc(x.c)}</strong><span>${x.growth>0?'+':''}${x.growth} · 최근 ${x.recent}</span></button>`).join('')||'<div class="dash-empty">산업 데이터가 없습니다.</div>'}</div></section>
    </div>`;
