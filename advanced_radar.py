...
def news(q):
 try:
  ...
  r=urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'Mozilla/5.0'}),timeout=12).read().decode('utf-8','ignore')
...
def run(items):
 ...
 for x in items:
  ...
  if hits:x['standaloneStatus']='기존 보도 가능성 있음'
  else:x['standaloneStatus']='검색상 동일 제목 기사 미확인'
  ...
  # 5) recurring vendors / institutions (when vendor text exists)
